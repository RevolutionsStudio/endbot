    ...
    for funct in self.funct: # for every command register
      if funct.__name__.split(" ")[0] == execute[0]: # Found the command
        ...
        if allow: # if role right to command AUTH
          ...
          await self.CONF.send_warn("Vous n'avez pas accès à cette commande.","Cette commande n'est utilisable seulement avec le role "+str(" ou ".join(funct.authGroup).lower()),self.message)
          return
